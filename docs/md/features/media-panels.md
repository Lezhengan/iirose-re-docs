...
## 六、媒体卡片 / 事件类型与分辨率（协议速查）

> 以下为「媒体卡片」与「媒体事件」的类型码、视频分辨率码，以及转义和地址前缀规则，全部来自真实抓包对照。
...
> 视频平台：**卡片用 `*`，事件用 `!`**；音频平台两者都用 `@`。

### 6.2 视频分辨率码

> 数字为服务端返回的分辨率码（点播卡片/事件里那个数字）。
...
> 大多数接口不需要转义，只有媒体卡片 `m` 字段里的**输入参数**（歌名/歌手/标题/作者等）需要转义。
...
- 原始内容：
```
{"m":"m__4@0>诀别书>邓垚>http://p2.music.126.net/wztA5smxFjIfv98u7-IrQQ==/109951168933355255.jpg>202c3a>128","mc":"202c3a","i":"460297465003"}
```
- 内容解析：
```
{"m":"m__4(类型码)>(歌名)>(歌手)>(封面图)>(主题色)>(码率)","mc":"(卡片颜色)","i":"(随机数)"}
```

### 6.4 媒体地址前缀

> 媒体事件 `&1{...}` 里的地址字段（`s` 媒体地址、`c` 封面、`o` 原链接）省略协议头：
...
- 内容解析：
```
&1{"s":"(媒体地址)","d":(时长秒),"c":"(封面)","n":"(名称)","r":"(作者)","b":"(类型码)"}
```

> 音乐平台媒体还多带 `o`（原链接，`s://` 开头）和 `l`（LRC 歌词）两个字段；视频平台媒体与自定义媒体一致，只有 `s/d/c/n/r/b`。
