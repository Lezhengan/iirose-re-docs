...
> 均为 GET/POST 明文请求，返回 JSON 字符串或原始文本。**域名按业务拆分**：`a.iirose.com`=API、`b.iirose.com`=业务、`f.iirose.com`=上传、`r.iirose.com`=回显前缀、`z.iirose.com`=辅助、`w.iirose.com`=代理、`d.iirose.com`=盾/支付、`s.iirose.com`=静态、`mx.iirose.com`=地域检测。各章节标题已标注完整基础 URL，直接拼接接口名即可。
...
| `socialAccGet.php` | POST | `v`=轮询ID | QQ 扫码登录轮询：QQ 登录窗弹出后每 5 秒 POST 一次轮询ID，成功返回扫码结果 |
| `socialAcc.php` | POST | `t`=类型, `k`=凭证, `d`=设备, `b`=是否beta | 社交账号绑定/登录（微信/QQ 等） |
| `wxJsSdk.php` | GET | — | 微信 JS-SDK 签名（返回空格分隔字符串，供 `wx.config`） |
| `countip.php`（域 `b.iirose.com`） | POST | `ss`=屏幕分辨率, `pb`=可视区, `ck`=Cookie, `device`/`promoterID`/`partner`/`ref` 可选 | 访问统计埋点，`countIp("*Login")`/`"*GuestLogin"`/`"*Language"` 只是事件标签，**不是 IP 限流** |

### 登录错误码 / 请求限流

登录页 4 个认证接口（账号登录/游客登录/找回用户名/重置密码）的错误返回约定：**单字符错误码**，`!` 特殊：
...
- 计数与封禁全在服务端：前端只在收到 `!` 时弹提示，源码中**没有任何本地计时/计次逻辑**（"24小时"仅文案，阈值不可见）
- 聊天页进房认证（`*`+JSON）被限流时返回错误码 `4` → `sessionStorage.loginError=4` 踢回登录页显示同一句
- `loginError` 全表：`1`=用户名不存在、`2`=密码错误、`3`=名字被占、`4`=当日次数上限
- 手机验证码申请（`$5`）是另一道限流，返回 `4{code}`：`1`=单 IP 每日最多 30 次、`2`=单账号每日最多 3 次、`4`=剩余次数提示
...
> 触发入口是 `Utils.service.moveinputDo("<> 链接")`。**部分平台不走 API，直接拼 WS 命令**（下表"方式"列）。

| 接口 | 参数 | 平台 | 触发链接特征 | 返回处理 |
|---|---|---|---|---|
| `search_163Music.php` | `s`=关键词, `l`=1, `p`=1 | 网易云搜索（`@@ 歌名`） | `@@ 歌名`（取第一首直发）；`@ 歌名` 则是打开媒体搜索面板走 demandHolder 多引擎 | `result.songs[0]`，`fee=1` 弹 VIP 提示，拼 `<> 链接` 发送 |
...
| `info_163Music_radio.php` | `i`=电台id, `n`=1 | 网易云电台/节目 | `163.com/#/dj`/`/program` | `program.mainSong.rurl`；`n=1` 仅出现在 getMediaLink 分享文本解析路径，`<> 链接` 分发器只传 `i` |
...
| `douban.php` | `c`=频道号 | 豆瓣FM 电台换台 | 豆瓣电台切台按钮 | `song[0]` 歌曲信息（标题/歌手/播放地址），前端 `radioPlayer` 播放 |
| `cors_media.php`（域 `z.iirose.com`） | `t`=类型, `s`=网易云歌曲id | 媒体 CORS 代理 | 媒体加载时自动代理：检测到 `music.163.com/song/media/outer/url?id=` 网易云外链即改写走该代理 | 代理后的可播 URL |
| `search_${type}.php` | 动态拼接 | 表情/媒体通用搜索：非 http 前缀的 `d` 值拼成 `api/search_{d}.php` | — | — |
| `search_emoji.php` | GET 关键词 | 表情搜索 | — | — |
| `translate.php` | POST `text` 等 | 文本翻译 | — | — |

**不走 API 直接拼 WS 命令的平台**（链接特征匹配后直发）：

- **腾讯视频**：从链接提取 coverid/vid → `socket.send("dv1"+id)`（`v.qq.com/x/cover/`、`/x/page/`、`m.v.qq.com/play`）
- **YouTube**：不走 PHP API，前端直接调 YouTube IFrame API 解析
...
| 接口 | 参数 | 说明 |
|---|---|---|
| `pay.php` | `i`=支付token | 支付跳转（弹窗/新页打开） |
...
| `icon.php` | — | `i`=头像名, `s`=1 | 头像图标数据（GET `{i}` 返回 `\|` 分隔的图标数据集；`s=1` 变体用于头像丢失重取并写回 Cookie） |
| `loadImgEdit.php` | `w.iirose.com`/`f.iirose.com` | `s`=编辑指令, `r`=尺寸 | 图片编辑/压缩加载（列表缩略图、主色调提取） |
| `loadImg.php` | `z.iirose.com` | `s`=图片URL | 图片防盗链代理加载（下载/转存） |
| `changes.php` | — | `v`=版本号, `l`=语言(0中/1英) | 更新日志动态数据（`\n#` 分节文本） |
| `lib/system/data/changes/changes_{cn\|en}` | 相对路径 | — | 更新日志静态数据（离线模式用） |
| `countip.php`（域 `b.iirose.com`） | POST | `ss`=屏幕尺寸, `pb`=窗口尺寸, `ck`=cookie, `ref`=来源页 | 页面访问统计 |
| `push.php` | POST | `act=reg` + 推送token | 推送服务注册（消息推送提醒） |
| `debug.php` | — | — | 调试接口 |
...
| 接口 | 参数 | 说明 |
|---|---|---|
| `lib/php/system/file_upload.php` | `i`=uid, `f[]`=文件 | 上传文件；返回相对路径文本（图片以 `i/` 开头） |
| — | — | `http://r.iirose.com/`（`Urls.uploadedPrefix.img`）= 回显前缀 |
...
- 客户端上传函数：`Utils.uploadImg(file, callback)`，内部 FormData 自动带 `i`=uid
- 拼接使用点：`Constant.URL.uploadedPrefixImg + e.responseText`
- 图片格式白名单：`jpg jpeg png gif bmp webp jfif apng avif`
- 文件/媒体/流上传共用同一接口，前缀均指向 `r.iirose.com/`
